...
def process_audio_and_chat():
    ...
    while True:
        try:
            ...
            response = ask_agent(user_query=text)

            voice_of_agent = text_to_speech_with_elevenlabs(input_text=response, output_filepath="final.mp3")
            
            chat_history.append([text, response])
            # response=ask_agent(chat_history)
            # chat_history.append({"role": "assistant", "content": response})
            # text_to_speech_with_elevenlabs(response, "audio_response.wav")
            # yield response, chat_history

        except Exception as e:
            print(f"An error occurred: {e}")
            break
        # yield "An error occurred. Please try again.", chat_history
